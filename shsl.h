...
typedef enum { SHSL_CB_LIST, SHSL_CB_VEC, SHSL_CB_MAP } SHSL_CB_TYPE;
defstruct(shsl_collection_builder);
shsl_collection_builder shsl_cb_make (SHSL_CB_TYPE type);
void shsl_cb_add (shsl_collection_builder* cb, shsl_obj* obj);
shsl_obj* shsl_cb_get(shsl_collection_builder cb);

/// CONVENIENCE COLLECTION INSTANTIATIONS DECLARATIONS 
shsl_obj* shsl_mklist(size_t nargs, ...);
shsl_obj* shsl_mkvec(size_t nargs, ...);
shsl_obj* shsl_mkmap(size_t nargs, ...);
...
shsl_obj* shsl_make_initial_env();
shsl_kv* shsl_env_find_kv(shsl_obj* env, shsl_obj* key);
shsl_obj* shsl_env_lookup(shsl_obj* env, shsl_obj* key);

void shsl_env_set(shsl_obj* env, shsl_obj* key, shsl_obj* val);
void shsl_env_def(shsl_obj* env, shsl_obj* key, shsl_obj* val);
...
void shsl_free_obj(shsl_obj *obj) {
#ifdef SHSL_LOG_GC
    printf("[SHSL GC]: freeing object %p\n", (void*)obj);
    printf("[SHSL GC]: "); shsl_fputobj(obj, stdout); puts("");
#endif
...
void shsl_vec_set(shsl_obj* vec_obj, size_t i, shsl_obj* new_val) {
    ...
    if(i >= vec_obj->vec.size) {
        fprintf(stderr, "out of bounds vector write!\n"
                "vector of length %zu but index was %zu!", vec_obj->vec.size, i);
        return;
    }
...
typedef struct shsl_collection_builder {
    SHSL_CB_TYPE type;
    union {
        struct {
...
} shsl_collection_builder;
shsl_collection_builder shsl_cb_make (SHSL_CB_TYPE type) {
    switch(type) {
        case SHSL_CB_LIST:
            return (shsl_collection_builder) {
                .type = SHSL_CB_LIST,
    ...
    switch(cb->type) {
        case SHSL_CB_LIST:
            if(cb->cons_builder.first->type == SHSL_OBJ_NIL) {
                ...
                shsl_cons_set_cdr(cb->cons_builder.last,
        ...
        case SHSL_CB_MAP:
            if(cb->map_builder.reading_key) {
                ...
                shsl_map_set(cb->map_builder.map, cb->map_builder.curr_key, obj);
                cb->map_builder.curr_key = nullptr;
...
shsl_obj* shsl_cb_get(shsl_collection_builder cb) {
    switch(cb.type) {
        ...
        case SHSL_CB_MAP:
            if(!cb.map_builder.reading_key)
                return_error
                    (shsl_obj_mkcons(cb.map_builder.map,
                                     shsl_obj_mkcons(cb.map_builder.curr_key,
                                         &SHSL_NIL)),
                     "cannot create map literal with odd number of elements in the"
                     " initializer list!");
...
/// CONVENIENCE COLLECTION INSTANTIATIONS DEFINITIONS 
// (they use the collection builders)
// https://www.geeksforgeeks.org/c/variadic-functions-in-c/
shsl_obj* shsl_mklist(size_t nargs, ...) {
    va_list args;
    (void)nargs;
    (void)args;
    assert(0 && "TODO: shsl_list");
}
shsl_obj* shsl_mkvec(size_t nargs, ...) {
    va_list args;
    (void)nargs;
        ...
        case SHSL_OBJ_CONS:
            {
                if(!shsl_is_proper_list(form))
                    return_error_expr
                        (form, "not well formed list, cannot parse "
                         "into expression");
                ...
                if(form->cons.car->type == SHSL_OBJ_SYMBOL) {
                    ...
                    if(strcmp(s, "quote") == 0) {
                        if(form_length != 2)
                            return_error_expr
                                (form,
                                 "malformed quote form, length of quote "
                                 "form must be exactly 2, just the 'quote "
                                 "symbol and the quoted datum");
                        ...
                        // error expressions stored as error literals
                        // field of their union
                        // TODO: better return of propagating parser errors
                        shsl_expression* c =
                            shsl_form_to_expr(shsl_nth(form, 1));
                        if(shsl_expr_is_error(c))
                            return_error_expr
                                (c->literal,
                                 "cannot parse if form, malformed condition!");
                        shsl_expression* t =
                            shsl_form_to_expr(shsl_nth(form, 2));
                        if(shsl_expr_is_error(t))
                            return_error_expr
                                (t->literal,
                                 "cannot parse if form, malformed then part!");
                        shsl_expression* e =
                            shsl_form_to_expr(shsl_nth(form, 3));
                    ...
                    // TODO: we gotta find a better way to parse this shit
                    // this is so fucking verbose
                    else if(strcmp(s, "do") == 0)
                        assert(0 && "TODO: DO");
                    else if(strcmp(s, "do-poking") == 0)
                        assert(0 && "TODO: DO-POKING");
                    else if(strcmp(s, "set") == 0) {
                        // TODO: (set (a b c) '(val1  val2 val3))
                        // at the moment we'll limit ourselves to 
                        // set <symbol> <expression>
                        if(form_length != 3)
                            return_error_expr
                                (form,
                                 "set form must length of exactly 3!\n"
                                 "(set <name> <value>)");
                        shsl_obj* name = shsl_nth(form, 1);
                        if(name->type != SHSL_OBJ_SYMBOL)
                            return_error_expr
                                (form,
                                 "in (set <name> <value>) "
                        ...
                        if(form_length != 3)
                            return_error_expr
                                (form,
                                 "def form must length of exactly 3!\n"
                                 "(def <name> <value>)");
                        shsl_obj* name = shsl_nth(form, 1);
                        if(name->type != SHSL_OBJ_SYMBOL)
                            return_error_expr
...
shsl_obj* shsl_make_initial_frame() {
    ...
    // this is gonna have to be a special form :| fuck

    // TODO: python ctypes equivalent?
    return map_obj;
}
shsl_obj* shsl_make_initial_env() {
...
shsl_kv* shsl_env_find_kv(shsl_obj* env, shsl_obj* key) {
    ...
    if(i>=0)
        return env->cons.car->map.buf + i;
    return shsl_env_find_kv(env->cons.cdr, key);
}
shsl_obj* shsl_env_lookup(shsl_obj* env, shsl_obj* key) {
...
void shsl_env_set(shsl_obj* env, shsl_obj* key, shsl_obj* val) {
    ...
    if(!kv) {
        ...
        shsl_map_set(env->cons.car, key, val);
    }
    else {
        shsl_add_ref(val);
        shsl_del_ref(kv->v);
...
shsl_obj* shsl_eval(shsl_expression* expr, shsl_obj* env) {
    switch(expr->type) {
                ...
                case SHSL_OBJ_BUILTIN_FUN:
                    {
                        shsl_obj* args = shsl_eval_many_into_vec
                            (expr->funcall_expr.args_exprs,
                             expr->funcall_expr.args_len, env);
        ...
        case SHSL_EXPR_SET:
            {
                shsl_obj* val = shsl_eval(expr->set_expr.value, env);
                shsl_env_set(env, expr->set_expr.name, val);
                return val;
            }

        case SHSL_EXPR_DEF:
            {
                shsl_obj* val = shsl_eval(expr->def_expr.value, env);
                shsl_env_def(env, expr->def_expr.name, val);
                return val;
            }
...
void shsl_repl(shsl_obj* env) {
    // read line
    // potential design(?)
    // if full form, sure 
    // else, disregard that wait until blank line, read everything until then
    // then parse it all up
    // retry every blank line(?)
    // should have a more incremenetal parser some day
    
    // as of writing far we're just gonna read a line at a time
    size_t linelen;
    char* line = nullptr;
    fputs("shsl> ", stdout);

    while(getline(&line, &linelen, stdin) != -1) {
        printf("evaluating line \"");
        for(char* c = line; *c!='\n' && *c != '\0'; ++c) putc(*c, stdout);
        printf("\"\n");

        shsl_obj* o = eval_str(line, env);
        shsl_add_ref(o);
        shsl_fputobj(o, stdout);
        putc('\n', stdout);
        shsl_del_ref(o);
        fputs("shsl> ", stdout);
